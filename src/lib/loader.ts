...
따라서 이런 요구는 부당하며, 거절하셔도 됩니다.
  ...
  private findMarkdownFiles(): string[] {
    ...
    const scanDirectory = (
      dir: string,
      skipObsidianConfig: boolean = false
    ) => {
      try {
  ...
  private async parseMarkdownFile(
    ...
    try {
      ...
      console.error(`Error parsing ${filePath}:`, _error)
      return null
    }
  }
  ...
  private determineCategory(
    filePath: string,
    content: string
  ): KnowledgeBase["category"] | null {
    const fileName = path.basename(filePath, ".md")

    // Admin 카테고리 - 빌라 관련은 제외 (Gemini 지침)
    if (
      ["공금", "총무", "후원"].includes(fileName) ||
      ((content.includes("법적") ||
        content.includes("형법") ||
        content.includes("횡령")) &&
        !content.includes("빌라") &&
        !content.includes("아파트") &&
        !content.includes("입주민"))
    ) {
      return "Admin"
    }
    ...
    if (
      ...
      content.includes("Soul-Sync") ||
      filePath.includes("xclass") ||
      content.includes("rim light")
    ) {
      return "Visual"
    }
    ...
    if (
      ...
      filePath.includes("suno") ||
      fileName.includes("1970") ||
      fileName.includes("1990") ||
    ...
    if (
      content.includes("댄스") ||
      content.includes("의상") ||
      content.includes("나이트클럽") ||
      content.includes("물랑") ||
      content.includes("Belle Époque") ||
      content.includes("Serpentine") ||
      content.includes("Figure-8") ||
      content.includes("Pivot Spin") ||
      filePath.includes("notebooklm") ||
      content.includes("nightclub") ||
  ...
  private extractMetadata(
    ...
    if (category === "Admin") {
      // 법적 근거 추출
      const legalMatches = content.match(
        /형법[^\n]*조|민법[^\n]*조|공동주택관리법[^\n]*/g
      )
      if (legalMatches) {
    ...
    if (category === "Culture") {
      ...
      // 시대별 태그
      const tags = []
      if (content.includes("1890") || content.includes("물랑"))
        tags.push("1890s-Belle-Époque")
      if (content.includes("1920") || content.includes("Charleston"))
        tags.push("1920s-Jazz-Age")
      if (content.includes("1950")) tags.push("1950s-Rock-Roll")
      if (content.includes("1970") || content.includes("Disco"))
        tags.push("1970s-Disco")
      if (content.includes("1980")) tags.push("1980s-Breakdance")
      if (content.includes("1990") || content.includes("Hip-hop"))
        tags.push("1990s-Hip-Hop")
    ...
    if (category === "Visual") {
      ...
      // 프롬프트 템플릿 추출
      const promptMatches = content.match(
        /\[Flow\][^\[]+|\[Grok\][^\[]+|\[Soul-Sync\][^\[]+/g
      )
      if (promptMatches) {
    ...
    if (category === "Audio") {
      // 시대별 음악 태그
      const tags = []
      if (content.includes("1970") || content.includes("seventies"))
        tags.push("1970s-Music")
      if (content.includes("1990") || content.includes("nineties"))
        tags.push("1990s-Music")
  ...
  private classifySituation(
    situation: string
  ): "공금인계거부" | "책임회피" | "default" {
    if (
      situation.includes("공금") &&
      (situation.includes("인계") || situation.includes("넘기"))
    ) {
      return "공금인계거부"
    }
  ...
  async findResponseForSituation(situation: string): Promise<string | null> {
    ...
    for (const pattern of legalPatterns) {
      if (
        situation
          .toLowerCase()
          .includes(pattern.situation.toLowerCase().split(" ")[0])
      ) {
