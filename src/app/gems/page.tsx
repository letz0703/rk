...
const gems = [
  ...
  {
    href: "https://gemini.google.com/gem-labs/1bRI9-b4qDujeAZ11Z7wib_or91wwHBT6",
    emoji: "🪬",
    title: "제갈공명",
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white px-4 py-12">
      <div className="max-w-3xl mx-auto space-y-8">
        <Link href="/" className="text-xs text-gray-500 hover:text-white transition">← Home</Link>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {gems.map(gem => {
            const cardClass = "group bg-[#141414] border border-white/10 rounded-2xl p-6 hover:border-white/30 hover:bg-[#1a1a1a] transition-all duration-200 flex flex-col gap-3"
            const inner = (
              <>
                <div className="flex items-start justify-between">
                  <span className="text-2xl">{gem.emoji}</span>
                  <div className="flex items-center gap-2">
                    {gem.locked && <span className="text-xs text-gray-600">🔒</span>}
            ...
            if (gem.locked) return (
              <button key={gem.href} onClick={() => handleLockedClick(gem.href)} className={`${cardClass} text-left`}>
                {inner}
              </button>
            )
            if (gem.internal) return (
              <Link key={gem.href} href={gem.href} className={cardClass}>
                {inner}
              </Link>
            )
            return (
              <a key={gem.href} href={gem.href} target="_blank" rel="noopener noreferrer" className={cardClass}>
                {inner}
              </a>
            )
          })}
