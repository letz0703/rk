  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-8 md:p-16">
      ...
      <div className="max-w-6xl mx-auto mb-16 flex flex-col md:flex-row justify-between items-end gap-8">
        <div>
          <h2 className="text-purple-500 font-mono tracking-widest uppercase mb-2">
            Portfolio
          </h2>
          ...
          <p className="text-slate-400 font-light max-w-lg">
            보라색 머리 소녀의 모든 착장과 스타일링 기록입니다. 그녀는 단순한
            가이드를 넘어, 유네스코의 문명을 투영하는 아이콘이 됩니다.
          </p>
        </div>
        <Link
          href="/travelogue"
              ...
              .map((img, index) => (
                ...
                >
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-6 bg-gradient-to-t from-black via-black/40 to-transparent flex flex-col justify-end translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                    <span
                      className={`text-[10px] font-black px-2 py-0.5 rounded-sm inline-block w-fit mb-2 uppercase tracking-tighter ${img.era === "Present" ? "bg-blue-500 text-white" : "bg-purple-600 text-white"}`}
                    >
                      {img.era}
                    </span>
                    <span className="text-xs text-slate-400 font-mono">
                      {img.timestamp === "Current"
                        ? "Latest Look"
                        : img.timestamp.split("_")[0] || "Unknown Date"}
                    </span>
                  </div>
      ...
      <div className="mt-32 text-center opacity-10 pointer-events-none">
        <span className="text-[15vw] font-black tracking-tighter select-none leading-none">
          LOOKBOOK
        </span>
      </div>
        ...
        >
          ...
          >
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 px-6 py-3 bg-black/60 backdrop-blur-md rounded-full flex items-center gap-4 pointer-events-none">
              <span
                className={`text-[10px] font-black px-2 py-0.5 rounded-sm uppercase tracking-tighter ${selectedImage.era === "Present" ? "bg-blue-500 text-white" : "bg-purple-600 text-white"}`}
              >
                {selectedImage.era}
              </span>
              <span className="text-xs text-slate-300 font-mono">
                {selectedImage.timestamp === "Current"
                  ? "LATEST"
