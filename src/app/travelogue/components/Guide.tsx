...
export default function Guide({message, timeMode}: GuideProps) {
  ...
  const [customGuideImages, setCustomGuideImages] = useState<
  ...
  return (
    <div
      className={`fixed bottom-10 right-10 z-50 flex flex-col items-end transition-all duration-1000 ${isVisible ? "translate-y-0 opacity-100" : "translate-y-20 opacity-0"}`}
    >
      {/* Speech Bubble */}
        ...
        >
          ...
          {customGuideImages[timeMode] ? (
            <Image
              ...
              fill
              unoptimized
              className="object-contain drop-shadow-[0_0_20px_rgba(168,85,247,0.4)] animate-in fade-in duration-500"
              onError={() => {
